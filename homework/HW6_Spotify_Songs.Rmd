...
```{r}
spotify_songs |> summarize(mean(danceability), sd = sd(danceability))
```
...
This distribution is not normal. The whole graph is filled in black, so I can't even read the distribution. I suppose this means that people listening to music on spotify have all different music tastes?
